...
def generate_mixed_data(n_samples, n_variables, discrete_ratio=0.5):
    ...
    # Generate discrete variables (binary values: 0, 1)
    discrete_data = np.random.randint(0, 2, size=(n_samples, n_discrete))
...
def save_dataset(df, filename):
    ...
    # Get arities for all variables
    arities = []
    for col in df.columns:
        if col.startswith('C'):  # Continuous variable
            arities.append(0)  # 0 indicates continuous variable
        else:  # Discrete variable
            # Count unique values, binary should be 2
            arities.append(2)  # Binary variable has arity 2
    
    # First write the arity line
    with open(filename, 'w') as f:
        f.write(' '.join(map(str, arities)) + '\n')
    
    # Then append the data
    df.to_csv(filename, sep=' ', index=False, header=True, mode='a')
...
def run_gobnilp(dataset_path, timeout=600):
    ...
    cmd = ["python", "rungobnilp.py", "--mec", "--score", "MixedLL", dataset_path]
...
def main():
    # Parameters for test
    n_samples = 1000
    variable_counts = [10]  # Number of variables to test
    ...
    if cleanup_files:
        cleanup_test_files(test_dir, keep_results=True)
